...
func (qb *JoinsQueryBuilder) CreatePerformerAliases(newJoins []PerformerAliases, tx *sqlx.Tx) error {
	ensureTx(tx)
	for _, join := range newJoins {
		_, err := tx.NamedExec(
			`INSERT INTO performer_aliases (performer_id, alias) VALUES (:performer_id, :alias)`,
			join,
		)
		if err != nil {
			return err
...
func (qb *JoinsQueryBuilder) UpdatePerformerAliases(performerID int, updatedJoins []PerformerAliases, tx *sqlx.Tx) error {
	ensureTx(tx)

	// Delete the existing joins and then create new ones
	_, err := tx.Exec("DELETE FROM performer_aliases WHERE performer_id = ?", performerID)
	if err != nil {
		return err
	}
	return qb.CreatePerformerAliases(updatedJoins, tx)
}

func (qb *JoinsQueryBuilder) DestroyPerformerAliases(performerID int, tx *sqlx.Tx) error {
	ensureTx(tx)
